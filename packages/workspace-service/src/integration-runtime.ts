...
export class IntegrationCoordinator {
  ...
  async zoteroCapability(profileId: string, secret: string | null = null): Promise<ZoteroCapabilityStatus> {
    ...
    try {
      ...
      const message = probe.message.toLocaleLowerCase()
      const status = probe.ok
        ? 'connected'
        : message.includes('授权') || message.includes('unauthor') || message.includes('权限')
          ? 'unauthorized'
          : message.includes('频繁') || message.includes('rate')
            ? 'rate_limited'
            : message.includes('支持') || message.includes('unsupported')
              ? 'unsupported'
              : 'unavailable'
      return ZoteroCapabilityStatusSchema.parse({ status, capability: { read: probe.capabilities.read, write: probe.capabilities.write }, checkedAt })
    } catch (error) {
      const message = error instanceof IntegrationRuntimeError ? error.message.toLocaleLowerCase() : ''
      const status = message.includes('授权') || message.includes('unauthor') ? 'unauthorized' : message.includes('频繁') || message.includes('rate') ? 'rate_limited' : 'unavailable'
      return ZoteroCapabilityStatusSchema.parse({ status, capability: { read: false, write: false }, checkedAt })
    }
  }
  ...
  async readNote(input: ReadNoteInput): Promise<Note> {
    ...
    return { ...note, id: `${profile.id}:${note.relativePath}`, vaultId: profile.id }
  }
  ...
  async previewZoteroImport(input: ZoteroImportPreviewInput & { secret?: string | null }): Promise<ZoteroImportPreview> {
    ...
    const items = direction === 'to-zotero'
      ? parsed.paperIds.map((paperId) => this.previewPaperToZoteroItem(profile.id, paperId))
  ...
  async previewPaperToZotero(input: PaperToZoteroPreviewInput & { secret?: string | null }): Promise<PaperToZoteroPreview> {
    ...
    if (stored !== undefined) this.zoteroPreviews.set(preview.previewId, { ...stored, tags: parsed.tags })
  ...
  async executeZoteroImport(input: ZoteroImportExecuteInput, secret: string | null = null, expectedProfileId?: string): Promise<ZoteroImportResult> {
    ...
    const items = preview.direction === 'to-zotero'
      ? await Promise.all(preview.paperIds.map((paperId) => this.writePaperToZotero({ profileId: preview.profileId, paperId, targetCollectionKey: preview.targetCollectionKey, format: preview.format, transport: preview.transport, tags: preview.tags }, secret)))
  ...
  private async writePaperToZotero(input: { profileId: string; paperId: string; targetCollectionKey: string | null; format: ZoteroImportFormat; transport: ZoteroImportTransport; tags?: readonly string[] }, secret: string | null): Promise<import('@prw/contracts').ZoteroImportReceipt> {
    ...
    // Keep the unbound case explicit so an exported/imported item can always
    // be filtered later (`#未分类` in the UI, `未分类` in Zotero's tag field).
    const projectTag = paper.projectId === null
      ? '未分类'
      : this.repository.listProjects().find((project) => project.id === paper.projectId)?.name ?? '未分类'
    ...
    let zoteroItemKey: string = paper.id
    ...
    try {
      ...
      zoteroItemKey = link?.externalId ?? paper.id
      zoteroRemoteRevision = link?.remoteRevision ?? null
      if (input.transport !== 'api') {
        return { profileId: input.profileId, itemKey: link?.externalId ?? paper.id, paperId: paper.id, outcome: 'generated', transport: input.transport, format: input.format, locator: null, remoteRevision: link?.remoteRevision ?? null, duplicateDecision: link === undefined ? 'create' : 'update-candidate', error: null }
      }
      const capability = await this.zoteroCapability(input.profileId, secret)
      if (!capability.capability.write) {
        throw new IntegrationRuntimeError('PERMISSION_DENIED', 'Zotero 未获得写入权限。请在设置中完成写入授权后重新导入。')
      }
      if (link === undefined) {
        const receipt = await createZoteroProjection(toAdapterProfile(profile, secret), input.targetCollectionKey, projectionForZotero(paper.id, paper.id))
        ...
        return { profileId: input.profileId, itemKey: receipt.externalId, paperId: paper.id, outcome: 'written', transport: 'api', format: input.format, locator: receipt.locator, remoteRevision: receipt.remoteRevision, duplicateDecision: 'create', error: null }
      }
      if (link.remoteRevision === null) {
        throw new IntegrationRuntimeError('REVISION_CONFLICT', '缺少 Zotero 条目版本。请先刷新 Zotero 条目，再重新预览导入。')
      }
      const receipt = await writeProjection(profile, toAdapterProfile(profile, secret), { externalId: link.externalId, locator: link.locator, remoteRevision: link.remoteRevision }, projectionForZotero(link.managedBlockId ?? link.id, link.externalId))
      this.repository.saveExternalLink({
        profileId: profile.id,
        entityKind: 'paper',
        entityId: paper.id,
      ...
      return { profileId: input.profileId, itemKey: link.externalId, paperId: paper.id, outcome: 'written', transport: 'api', format: input.format, locator: receipt.locator, remoteRevision: receipt.remoteRevision, duplicateDecision: 'update-candidate', error: null }
    } catch (error) {
      return this.zoteroReceiptError(input.profileId, zoteroItemKey, input.format, input.transport, error, paper.id, zoteroRemoteRevision)
    }
  }
  ...
  private zoteroReceiptError(profileId: string, itemKey: string, format: ZoteroImportFormat, transport: ZoteroImportTransport, error: unknown, paperId: string | null = null, remoteRevision: string | null = null): import('@prw/contracts').ZoteroImportReceipt {
...
function writeProjection(
  profile: IntegrationProfile,
  adapterProfile: AdapterProfile,
  target: { externalId: string; locator: string; remoteRevision: string | null },
