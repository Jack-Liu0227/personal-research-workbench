...
export function TaskWorkspacePage({ projects, selectedProjectId, onSelectProject, workspaceTimezone = 'Asia/Shanghai', onContextMenu: shellOnContextMenu, focusTaskId = null }: { projects: Project[]; selectedProjectId: string | null; onSelectProject: (id: string | null) => void; workspaceTimezone?: string; onContextMenu?: TaskContextMenuHandler; focusTaskId?: string | null }): React.JSX.Element {
  ...
  // The bulk payload only ever contains tasks that are still in the filtered
  // list, so the visible count must be the same intersection: switching a
  // filter must never show a count that no button can act on.
  const explicitSelectedCount = tasks.filter((task) => selectedIds.has(task.id)).length
  const selectedCount = selectionMode === 'all-results' ? tasks.length - excludedIds.size : selectionMode === 'page' ? tasks.length : explicitSelectedCount
