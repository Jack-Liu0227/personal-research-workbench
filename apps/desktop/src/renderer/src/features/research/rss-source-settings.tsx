  ...
  return <>
    <ResearchPanel eyebrow="RSS / INTELLIGENCE SOURCES" title="RSS 订阅源（每日文献推送）">
      ...
      <form className="grid gap-3 p-4" onSubmit={submit}>
        ...
        <div className="grid gap-3 sm:grid-cols-2"><Field htmlFor="rss-title" label="来源名称"><Input id="rss-title" maxLength={120} onChange={(event) => setTitle(event.target.value)} placeholder="解析后自动填充" value={title} /></Field><Field htmlFor="rss-site-url" label="网站"><Input id="rss-site-url" onChange={(event) => setSiteUrl(event.target.value || null)} placeholder="解析后自动填充" value={siteUrl ?? ''} /></Field></div>
        ...
        <div className="flex flex-wrap items-end gap-2"><Field htmlFor="rss-category" label="分类"><select className="select-control" id="rss-category" onChange={(event) => setCategoryId(event.target.value)} value={categoryId}>{(categories.data ?? []).map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></Field><Button loading={save.isPending} type="submit">{editing ? '保存修改' : '添加来源'}</Button>{editing ? <Button onClick={resetForm} type="button" variant="ghost">取消编辑</Button> : null}</div>
